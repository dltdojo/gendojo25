...
import { dao, Task } from "@/app/todos/todo-dao"; // Import Todo DAO (Data Access Object) for database interactions
...
export async function updateTask(task: Task, previousState: any, formData: FormData) {
  console.log('updateTodo bind task:', JSON.stringify(task));
  console.log('updateTodo from data:', formData);
  // Extract the title from the form data
  dao.updateTask(task);
  redirect("/todos");
}
