...
class TaskDAO {
  ...
  getAllTasks(): Task[] {
    ...
    `);
    return parseTasks(stmt.all() as TaskSqlite[]);
  }
  ...
  updateTask(task: Task): boolean {
    ...
    const stmt = this.db.prepare(`
      ...
      WHERE id = ?
    `);
    const result = stmt.run(title, completed ? 1 : 0, JSON.stringify(tags), id);
  ...
  addTag(id: number, tag: string): boolean {
    ...
    const result = stmt.run(JSON.stringify(newTags), id);
    return result.changes > 0;
  }
...
export const dao = new TaskDAO('todo.db');