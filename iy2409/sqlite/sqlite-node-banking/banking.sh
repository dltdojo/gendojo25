...
if [ ! -f "$DB_FILE" ]; then
  sqlite3 "$DB_FILE" "CREATE TABLE accounts (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, balance REAL DEFAULT 0.0);"
  sqlite3 "$DB_FILE" "CREATE TABLE transactions (id INTEGER PRIMARY KEY AUTOINCREMENT, source_id INTEGER, dest_id INTEGER, amount REAL, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP);"
fi
